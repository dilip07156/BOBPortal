//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace CardHolder.DAL
{
    using System;
    using System.Collections.Generic;
    
    public partial class Dispatch_Dtl
    {
        public long Dispatch_Dtl_Id { get; set; }
        public System.DateTime Dispatch_Dt { get; set; }
        public long CardHolder_Id { get; set; }
        public string CardHolder_nm { get; set; }
        public string CreditCard_num { get; set; }
        public string CreditCard_Acc_num { get; set; }
        public string Type { get; set; }
        public string Status_Pin_Card { get; set; }
        public string Dispatch_Mode { get; set; }
        public string AirwayBill_num { get; set; }
        public string Status_Delivery { get; set; }
        public long Created_by { get; set; }
        public System.DateTime Created_dt { get; set; }
        public Nullable<long> Updated_by { get; set; }
        public Nullable<System.DateTime> Updated_dt { get; set; }
        public string IP_Address { get; set; }
    
        public virtual CardHolder_Mst CardHolder_Mst { get; set; }
    }
}
